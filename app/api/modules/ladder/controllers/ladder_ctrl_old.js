...
function updateUserCompPoints(req, res) {
    async function asy_init() {
        try {
            ...
            let updateUserdata = async function calculatePoints(userList, compList, compNo, totalComp){
                ...
                for(let m=0; m<compList.data.length; m++){
                    ...
                    let condition = { 
                        ...
                        for(let i=0; i<userList.data.length; i++){
                            ...
                            updateUserdata(userList.data[i], compList.data[m], m+1, compList.data.length);
...
function updateUserCompGamePoints(req, res) {
    async function asy_init() {
        try {
            ...
            let updateUserdata = async function calculatePoints(userList, compList, currentRound, currentRoundGames, isFirstGame, isEnd){
                let userData = {
                    user: {},
                    currentRoundPoints: 0.00,
                    previousRoundPoints: 0.00,
                ...
                for(let m=0; m<compList.data.length; m++){
                    ...
                    let condition = { 
                        ...
                        for(let i=0; i<userList.data.length; i++){
                            ...
                            if(i == 0){
                                console.log("START LOOP TIME: ", new Date());
                            }

                            if(i == userList.data.length-1){
                                console.log("END LOOP TIME: ", new Date());
                            }


                            updateUserdata(userList.data[i], compList.data[m], currentRound, currentRoundGames, isFirstGame, (i == userList.data.length-1) ? true : false);
